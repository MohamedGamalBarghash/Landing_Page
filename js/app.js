...
for (let i = 0; i < sectionsArray.length; i++) {
    ...
    sectionButton.addEventListener('click', function () {
        ...
        let an;
        // set it to active
        sectionsArray[o].classList.add("your-active-class");
        sectionButton.classList.add("highlight");
...
function changeActive () {
    ...
    for (let i = 0; i < sectionsArray.length; i++) {
        ...
        if (scroll >= height - 200 && scroll <= height + sectionsArray[i].getBoundingClientRect().height - 200) {
            ...
            document.querySelectorAll(".sectionHolderBox")[i].classList.add("highlight");
        } else {
            sectionsArray[i].classList.remove("your-active-class");
            document.querySelectorAll(".sectionHolderBox")[i].classList.remove("highlight");
        }
    }
}